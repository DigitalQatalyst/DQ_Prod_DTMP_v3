...
import LCStage2Overview from "./lifecycle/LCStage2Overview";
...
import PMOverviewPage from "./portfolio/PMOverviewPage";
import PMMyRequestsPage from "./portfolio/PMMyRequestsPage";
import PMMyReportsPage from "./portfolio/PMMyReportsPage";
import PMSavedViewsPage from "./portfolio/PMSavedViewsPage";
import PMStage3Page from "./portfolio/PMStage3Page";
...
export default function Stage2AppPage() {
  ...
  const [activeSubService, setActiveSubService] = useState<string | null>(() => {
    if (marketplace === "portfolio-management") {
      const pmCardMap: Record<string, string> = {
        "my-requests": "my-requests",
      ...
      return cardId ? (pmCardMap[cardId] ?? "pm-overview") : "pm-overview";
    }
    if (
  ...
  useEffect(() => {
    ...
    if (marketplace === "portfolio-management" && cardId) {
      // Map incoming cardId to the correct PM sub-service id
      const pmCardMap: Record<string, string> = {
        "my-requests": "my-requests",
        "my-reports": "my-reports",
        "saved-views": "saved-views",
      ...
      setActiveSubService(pmCardMap[cardId] ?? "pm-overview");
      return;
    }
              ...
              {activeService === "Portfolio Management" ? (
                <div className="space-y-1">
                  {[
                    ...
                    { id: "pm-stage3", label: "TO Operations" },
                  ].map((item) => (
                    <button
                      key={item.id}
                      onClick={() => handleSubServiceClick(item.id)}
                      className={`w-full flex items-center gap-2 px-3 py-2 text-sm rounded-lg transition-colors ${
                        activeSubService === item.id
                ...
                <div className="space-y-4">
                  <div>
                    <h3 className="text-sm font-medium text-gray-900 mb-3">Lifecycle Workspace</h3>
                    <div className="space-y-2">
                      <button
                      ...
                      >
                        <div className="text-left">
                          <div className="font-medium">Overview</div>
                          <div className="text-xs text-gray-500 mt-0.5">Request and delivery KPIs</div>
                        </div>
                      </button>

                      <button
                        onClick={() => handleSubServiceClick('my-requests')}
                        className={`w-full flex items-start gap-3 p-3 text-sm rounded-lg transition-colors ${
                          activeSubService === 'my-requests'
                            ? "bg-orange-50 text-orange-700 border border-orange-200"
                            : "text-gray-700 hover:bg-gray-50 border border-transparent"
                        }`}
                      >
                        <div className="text-left">
                          <div className="font-medium">My Requests</div>
                          <div className="text-xs text-gray-500 mt-0.5">Your submitted service tracker</div>
                        </div>
                      </button>

                      <button
                        onClick={() => handleSubServiceClick('my-initiatives')}
                        className={`w-full flex items-start gap-3 p-3 text-sm rounded-lg transition-colors ${
                          activeSubService === 'my-initiatives'
                            ? "bg-orange-50 text-orange-700 border border-orange-200"
                            : "text-gray-700 hover:bg-gray-50 border border-transparent"
                        }`}
                      >
                        <div className="text-left">
                          <div className="font-medium">My Initiatives</div>
                          <div className="text-xs text-gray-500 mt-0.5">Your initiative shortcuts</div>
                        </div>
                      </button>

                      <button
                        onClick={() => handleSubServiceClick('my-reports')}
                        className={`w-full flex items-start gap-3 p-3 text-sm rounded-lg transition-colors ${
                          activeSubService === 'my-reports'
                            ? "bg-orange-50 text-orange-700 border border-orange-200"
                            : "text-gray-700 hover:bg-gray-50 border border-transparent"
                        }`}
                      >
                      ...
                      <button
                        onClick={() => handleSubServiceClick('compliance')}
                        className={`w-full flex items-start gap-3 p-3 text-sm rounded-lg transition-colors ${
                          activeSubService === 'compliance'
                            ? "bg-orange-50 text-orange-700 border border-orange-200"
                            : "text-gray-700 hover:bg-gray-50 border border-transparent"
                        }`}
                      >
                        <div className="text-left">
                          ...
                          <div className="text-xs text-gray-500 mt-0.5">Architecture, security &amp; regulatory checks</div>
                        </div>
                      </button>

                      <button
                        onClick={() => handleSubServiceClick('retirement')}
                        className={`w-full flex items-start gap-3 p-3 text-sm rounded-lg transition-colors ${
                          activeSubService === 'retirement'
                            ? "bg-orange-50 text-orange-700 border border-orange-200"
                            : "text-gray-700 hover:bg-gray-50 border border-transparent"
                        }`}
                      >
                        <div className="text-left">
                          ...
                          <div className="text-xs text-gray-500 mt-0.5">Decommission register &amp; migration paths</div>
                        </div>
                      </button>

                      <button
                        onClick={() => handleSubServiceClick('products')}
                        className={`w-full flex items-start gap-3 p-3 text-sm rounded-lg transition-colors ${
                          activeSubService === 'products'
                            ? "bg-orange-50 text-orange-700 border border-orange-200"
                            : "text-gray-700 hover:bg-gray-50 border border-transparent"
                        }`}
                      >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden h-full">
        ...
        <div className="flex-1 bg-gray-50 overflow-y-auto">
          {activeService === "Portfolio Management" && activeSubService ? (
            ...
            <div className="h-full overflow-y-auto">
              {activeSubService === "pm-overview" && <PMOverviewPage />}
              {activeSubService === "my-requests" && <PMMyRequestsPage />}
              {activeSubService === "my-reports" && <PMMyReportsPage />}
              {activeSubService === "saved-views" && <PMSavedViewsPage />}
              {activeSubService === "pm-stage3" && <PMStage3Page />}
            </div>
